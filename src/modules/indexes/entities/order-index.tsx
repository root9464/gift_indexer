...
export const OrderIndex = ({ icon, title, mcap, price, change_price, type, order_book_address }: OrderIndexProps) => {
  return (
    <OrderIndexPopup 
      trigger={<div className='flex cursor-pointer items-center justify-between rounded-xl border border-transparent px-4 py-4 transition-colors hover:border-gray-200 hover:bg-gray-50'>
      ...
      </div>} icon={icon} title={title} mcap={mcap} price={price} change_price={change_price}  type={type} order_book_address={order_book_address} />
  );
};
